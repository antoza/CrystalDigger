...
def generate_walls(shader, rotation=rotate((0, 0, 1), 0), n_x=10, n_y=10):
    offset_x = n_x / 2
    ...
    for i in range(-1, n_x + 1):
        ...
        Wall.add(Surface(shader, n_x=scale_x, n_y=scale_y))
        Walls.add(Wall)
    return Walls
...
def generate_scene(shader):
    ...
    left_walls = generate_walls(shader, n_x=n_x, n_y=n_y, rotation=rotate((0, 0, 1), 90))
    lower_walls = generate_walls(shader, n_x=n_x, n_y=n_y, rotation=rotate((0, 0, 1), 180))
